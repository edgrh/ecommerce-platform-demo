# 分布式电商演示平台 - 课程大作业说明与答辩准备

> 本文档按课程要求组织：背景、功能、技术方案、实现流程、结果展示说明、心得撰写参考、答辩题库。若 Word 版仍保留教师下发的说明性红字，定稿前自行删除即可。
...
本项目的社会价值在于：**以最小可运行闭环展示"如何在不牺牲可理解性的前提下，用成熟组件组合缓解峰值压力"**。它并非真实生产系统，但能帮助学习者理解：网关如何保护下游、缓存与锁如何配合避免超卖、消息队列如何把"非用户强感知"的工作异步化，从而提升整体吞吐与可用性。对掌握分布式技术的意义在于：将"通信 / 状态 / 一致性边界"三件事串起来：**通信**（HTTP、MQ）、**状态**（MySQL 分表、Redis）、**边界**（同步下单只做核心写路径，履约与风控走异步），便于在答辩中用自己的话解释为什么这样拆、 trade-off 在哪里。
...
前端路由对应页面：`/` 首页（商品）、`/seckill` 秒杀、`/orders` 订单（需登录）、`/login` 登录、`/merchant` 商家后台（商家角色）。共 **5** 个独立视图，满足"主页面不少于 5 个"的要求。
...
**考虑**：课程环境优先"可一键拉起、可答辩演示"，故未强制引入 Nacos 等服务发现，网关路由静态指向本机业务端口；分表采用 **应用层路由 + MyBatis**，避免部分网络环境下中间件依赖拉取失败。生产可演进为注册中心 + ShardingSphere 等，与课堂结论"接口稳定、路由策略可替换"一致。
...
**（2）消息队列：异步与削峰**  
下单主事务成功后发布消息：`order.created` 驱动履约任务写入与后续状态更新；`order.risk` 驱动风控审核任务落库。作用有三：缩短用户感知的同步 RT；用消费速率平滑写入峰值；将风控、出库等非强一致同步环节与核心下单解耦（最终一致语义需在答辩中说清：同步边界到哪里为止）。
...
以上组合在答辩中可概括为：**同步路径做"必须且短"的事，异步路径做"可恢复、可重试"的事，Redis 扛热点，DB 做权威账本**。
...
使用 `docker-compose`（或课程提供脚本）启动 MySQL、Redis、RabbitMQ。执行 `bcommerce-commerce/src/main/resources/schema.sql` 初始化表与种子数据（类目、风控规则等）。分别启动 **网关**（默认 8080）与 **业务服务**（默认 8081），前端开发服务器代理或直连网关基地址，保证浏览器请求走统一入口。
...
课程要求"不少于三张截图"。本地运行后自行截图，下列为每张图建议配的文字说明要点。
...
通过本门课程对分布式技术的学习，理解了 **CAP 在工程中的折中**：例如用异步 MQ 换取更短的主链路延迟，接受风控状态的最终一致；理解了 **消息传递模型**（点对点与发布订阅）在履约与风控解耦中的作用；理解了 **分片策略**（哈希取模）对查询与扩容的影响。结合本项目，把网关、缓存、锁、MQ、分表从幻灯片落实为可运行代码，答辩时能指着模块图说明数据流与故障降级路径，比单纯背诵定义更有效。

### 6.3 学习过程感悟（约 280 字）

非技术方面的收获包括：需求边界管理（Demo 与生产的区别）、文档化表达（让别人能复现你的环境）、时间分配（先打通主链路再补治理）。遇到依赖下载或编码问题时，学会了查日志、隔离问题（先验证 DB 再验证 MQ），而不是随机改配置。答辩准备过程也训练了"结论先行、再举例子"的表达方式，对后续实习与团队沟通都有帮助。
